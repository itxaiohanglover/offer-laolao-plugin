...
[![Edge 扩展商店](https://img.shields.io/badge/Edge-扩展商店-blue?logo=microsoftedge)](https://microsoftedge.microsoft.com/addons/detail/bhebbhnjfageofcfejjbgneclnmakpgp)
...
- **模板操作**：
  ...
  - 📋 复制模板
  - 🗑️ 删除模板（至少保留一个）
- **智能迁移**：自动将旧版单简历数据迁移到新的多模板系统
- **独立存储**：每个模板独立保存完整的简历数据
...
- **信任制验证**：给我们的 GitHub 项目点 Star 即可解锁高级功能
- **一次解锁**：确认 Star 后永久解锁所有高级功能
- **友好提示**：优雅的弹窗引导用户支持项目
- **涉及功能**：AI 简历优化等高级功能需要 Star 验证
...
2. **安装依赖并构建**
   ...
   构建完成后会生成 `build/chrome-mv3-prod` 文件夹
...
1. 访问 [Microsoft Edge 扩展商店](https://microsoftedge.microsoft.com/addons/detail/bhebbhnjfageofcfejjbgneclnmakpgp)
2. 点击「获取」按钮安装
3. 安装完成后，扩展图标将出现在浏览器工具栏

### 方式三：Edge 浏览器开发者模式

Edge 浏览器同样支持加载未打包的扩展：

1. 按照方式一的步骤 1-2 下载并构建项目
...
1. 在「📝 简历填写」标签页顶部，找到模板选择器
2. **切换模板**：使用下拉菜单选择不同的简历模板
3. **新建模板**：点击「+」按钮创建新的空白模板
4. **重命名模板**：点击「✏️」按钮修改模板名称
5. **复制模板**：点击「📋」按钮复制当前模板
6. **删除模板**：点击「🗑️」按钮删除模板（至少需要保留 2 个模板才可删除）
...
3. 点击「✨ AI 一键优化简历」按钮
4. **首次使用**：会弹出提示框，引导您给我们的 GitHub 项目点 Star 以解锁该功能
   - 点击 GitHub 链接前往项目页面点 Star
   - 点击「我已完成 Star，解锁功能」确认
5. 系统会逐个优化所有描述性内容
6. 优化完成后自动填充回表单
